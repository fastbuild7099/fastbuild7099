const e = '【可点击的选择框】',
    t = /<roleplay_options>\s*(?:```.*\n)?([\s\S]*?)(?:\n```)?\s*<\/roleplay_options>/im;
var o, n;

// 【核心修正函數】
async function a(mesId) {
    // 使用一個微小的延遲來避開與其他腳本（如MVU）的渲染衝突
    setTimeout(() => {
        const displayedMessage = retrieveDisplayedMessage(mesId);

        // 如果消息的DOM元素不存在，或已經被我們的腳本處理過，就直接返回
        if (displayedMessage.length === 0 || displayedMessage.find('.beautified-options-container').length > 0) {
            return;
        }

        const chatMessage = getChatMessages(mesId)[0];
        if (!chatMessage || !chatMessage.message) return;

        const optionsMatch = chatMessage.message.match(t);

        // 如果在消息文本中找到了 roleplay_options
        if (optionsMatch) {
            // 找到並移除原始未經美化的 <pre> 塊
            const rawPreBlock = displayedMessage.find('pre:contains("<roleplay_options>")');
            if (rawPreBlock.length > 0) {
                rawPreBlock.remove();
            }

            // 創建美化後的元素
            const beautifiedElement = n.extract_roleplay_options_element(optionsMatch[1]);
            // 給最外層容器添加一個唯一的標識符，用於防止重複渲染
            beautifiedElement.addClass('beautified-options-container');

            // 將美化後的元素追加到消息末尾
            displayedMessage.append(beautifiedElement);
        }
    }, 0); // 延遲0毫秒，將執行推到下一個事件隊列
}


async function r() {
    $('#chat').children('.mes[is_user=\'false\'][is_system=\'false\']').each((e, t) => {
        a(Number(t.getAttribute('mesid')))
    })
}

function s() {
    const e = $('.mes[is_user="false"][is_system="false"]').last().find('.roleplay_options_content').map((e, t) => $(t).text().trim()).toArray();
...
function(t) {
    ...
    t.update = async function() {
    ...
    }, t.extract_roleplay_options_element = function(e) {
        const t = $('<div class="roleplay_options">');
        return t.append(n), t.append($('<div class="roleplay_options_back">').append([...e.matchAll(/(.+?)[:：]\s*(.+)/gm)].map(e => ({
            title: e[1],
            content: e[2].replace(/^\$\{(.+)\}$/, '$1').replace(/^「(.+)」$/, '$1')
        })).map(({
            title: e,
            content: t
        }) => $('<div class="roleplay_options_item" tabindex="1">').on('click', function() {
            ! async function(e) {
                if (e.parents('.last_mes').length > 0) {
                    const t = e.find('.roleplay_options_content').text().trim();
                    if ('直接发送' === o.option.input_mode) triggerSlash(`/send ${t} || /trigger`);
                    else if ('覆盖输入' === o.option.input_mode) triggerSlash(`/setinput ${t}`);
                    else if ('尾附输入' === o.option.input_mode) {
        ...
        }).append(`<span class="roleplay_options_title"><strong>${e}</strong></span>`).append('<hr class="roleplay_options_hr">').append(`<span class="roleplay_options_content">${t}</span>`)))), t
    }
}(n || (n = {})), $(async () => {
    await errorCatched(o.update)(), await errorCatched(n.update)(), await r(), eventOn(tavern_events.CHAT_CHANGED, errorCatched(r)), eventOn(tavern_events.CHARACTER_MESSAGE_RENDERED, errorCatched(a)), eventOn(tavern_events.MESSAGE_UPDATED, errorCatched(a)), eventOn(tavern_events.MESSAGE_SWIPED, errorCatched(a)), eventOn(tavern_events.MESSAGE_DELETED, () => setTimeout(errorCatched(r), 1e3)), eventOn(tavern_events.WORLDINFO_UPDATED, errorCatched(async t => {
        t === e && (await o.update() || await n.update()) && await r()
    }))
});
let i = null;

function l() {
    null !== i && (setTimeout(s, _.get(getVariables({
        type: 'global'
    }), [e, '自动推进发送间隔'], 3e3)), ++i, i === _.get(getVariables({
        type: 'global'
    }), [e, '自动推进循环次数'], -1) && p())
}

function p() {
    eventRemoveListener(tavern_events.CHARACTER_MESSAGE_RENDERED, l), i = null, toastr.success('已停止自动推进', e)
}
$(async () => {
    eventOnButton('设置循环次数', async () => {
        const t = Number(await SillyTavern.callGenericPopup('设置循环次数 (-1 为直到按下 \'停止自动推进\')', SillyTavern.POPUP_TYPE.INPUT, _.get(getVariables({
            type: 'global'
        }), [e, '自动推进循环次数'], '-1'))); - 1 !== t && t <= 0 ? toastr.error('循环次数要么是 -1, 要么是大于 0 的整数') : (insertOrAssignVariables({
            [e]: {
                自动推进循环次数: t
            }
        }, {
        ...
        null === i ? (i = 0, l(), eventOn(tavern_events.CHARACTER_MESSAGE_RENDERED, l), toastr.success('已开启自动推进', e)) : toastr.error('自动推进在之前已开启, 请先停止自动推进')
    }), eventOnButton('停止自动推进', p)
});
