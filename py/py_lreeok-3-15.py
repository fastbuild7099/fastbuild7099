...
class Spider(Spider):
    ...
    async def fetch_detail(self, session, vod_id):
        ...
        try:
            async with session.get(f'{self.home_url}/voddetail/{vod_id}.html', headers=self.headers, timeout=aiohttp.ClientTimeout(total=10)) as res:
                ...
                def extract_fallback(xpath, default=""):
                    result = root.xpath(xpath)
                    return result[0].strip() if result else default

                vod_name = extract_fallback('//h3[@class="slide-info-title hide"]/text()', '')
                vod_pic = extract_fallback('//div[contains(@class, "vod-img")]//img/@data-src', '')
                vod_class = " ".join(root.xpath('//div[@class="info-parameter"]//li[contains(., "类型")]/a/text()')) if root.xpath('//div[@class="info-parameter"]//li[contains(., "类型")]/a/text()') else ''
                vod_year = extract_fallback('//div[@class="info-parameter"]//li[contains(., "年份")]/text()[last()]', '')
                vod_area = extract_fallback('//li[contains(., "地区：")]/text()[last()]', '大陆')
                if vod_area == '大陆':
                    vod_area = extract_fallback('//li/em[contains(., "地区：")]/following-sibling::text()[1]', '大陆')
                vod_remarks = extract_fallback('//div[@class="info-parameter"]//li[contains(., "状态")]/span/text()', '')
                vod_director = extract_fallback('//div[@class="info-parameter"]//li[contains(., "导演")]/span/text()', '未知')
                vod_actor = extract_fallback('//div[@class="info-parameter"]//li[contains(., "主演")]/span/text()', '未知')
                vod_lang = extract_fallback('//div[@class="info-parameter"]//li[contains(., "语言")]/text()[last()]', '国语')
                vod_content = extract_fallback('//div[@id="height_limit"]/text()', '暫無簡介')
    ...
    async def async_categoryContent(self, cid, page, filter, ext):
        ...
        try:
                ...
                if tasks:
                    ...
                    with self.cache_lock:
                        for result in results:
                            if isinstance(result, dict) and 'vod_id' in result:
                                self.detail_cache[result['vod_id']] = result
                                print(f"緩存更新: vod_id={result['vod_id']}, vod_area={result['vod_area']}")
            ...
            for item in data:
                ...
                print(f"檢查緩存: vod_id={vod_id}, cached_vod_area={cached_item.get('vod_area')}, api_vod_area={item.get('vod_area', '大陆')}, final_vod_area={vod_area}")

                area_filter = ext.get('area', '')
                area_match = (not area_filter or area_filter in vod_area)
                print(f"項目: {item.get('vod_name')}, vod_area: {vod_area}, ext['area']: {area_filter}, area_match: {area_match}")
    ...
    def detailContent(self, did):
        ...
        try:
            ...
            vod_class = " ".join(root.xpath('//div[@class="info-parameter"]//li[contains(., "类型")]/a/text()')) if root.xpath('//div[@class="info-parameter"]//li[contains(., "类型")]/a/text()') else ''
            vod_year = extract_fallback(root, '//div[@class="info-parameter"]//li[contains(., "年份")]/text()[last()]', '')
            vod_area = extract_fallback(root, '//li[contains(., "地区：")]/text()[last()]', '大陆')
            if vod_area == '大陆':
                vod_area = extract_fallback(root, '//li/em[contains(., "地区：")]/following-sibling::text()[1]', '大陆')
            vod_remarks = extract_fallback(root, '//div[@class="info-parameter"]//li[contains(., "状态")]/span/text()', '')
            vod_director = extract_fallback(root, '//div[@class="info-parameter"]//li[contains(., "导演")]/span/text()', '未知')
            vod_actor = extract_fallback(root, '//div[@class="info-parameter"]//li[contains(., "主演")]/span/text()', '未知')
            vod_lang = extract_fallback(root, '//div[@class="info-parameter"]//li[contains(., "语言")]/text()[last()]', '国语')
            vod_content = extract_fallback(root, '//div[@id="height_limit"]/text()', '暫無簡介')
            ...
            with self.cache_lock:
                self.detail_cache[ids] = video_item
