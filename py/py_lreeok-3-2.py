...
class Spider(Spider):
    def __init__(self):
        ...
        self.detail_cache = {}  # 緩存詳情數據，避免重複請求
    ...
    def homeContent(self, filter):
        return {
            ...
            'filters': {
                ...
                '2': [
                    {'key bubble': 'class', 'name': '类型', 'value': [{'n': '全部', 'v': ''}, {'n': '古装', 'v': '古装'}, {'n': '战争', 'v': '战争'}, {'n': '青春偶像', 'v': '青春偶像'}, {'n': '喜剧', 'v': '喜剧'}, {'n': '家庭', 'v': '家庭'}, {'n': '犯罪', 'v': '犯罪'}, {'n': '动作', 'v': '动作'}, {'n': '奇幻', 'v': '奇幻'}, {'n': '剧情', 'v': '剧情'}, {'n': '历史', 'v': '历史'}, {'n': '经典', 'v': '经典'}, {'n': '乡村', 'v': '乡村'}, {'n': '情景', 'v': '情景'}, {'n': '商战', 'v': '商战'}, {'n': '网剧', 'v': '网剧'}, {'n': '其他', 'v': '其他'}]},
            ...
            for item in data:
                ...
                if vod_id not in self.detail_cache:
                    ...
                    if detail_result['list']:
                        ...
                        continue  # 如果詳情獲取失敗，跳過該項目
                
                detail = self.detail_cache[vod_id]
                vod_class = item.get('vod_class', '')
                vod_area = detail.get('vod_area', '')
                vod_year = str(detail.get('vod_year', ''))  # 轉為字符串
                vod_lang = detail.get('vod_content', '')  # 從簡介中推測語言
    ...
    def detailContent(self, did):
        ...
        try:
            ...
            res.encoding = 'utf-8'
            root = etree.HTML(res.text)
            print(f"HTML content preview: {res.text[:500]}")  # 檢查頁面內容

            # 改進標題提取，增加穩健性
            vod_name_candidates = (
                root.xpath('//h1[@class="title"]/text()') or
                root.xpath('//h3[@class="slide-info-title"]/text()') or
                root.xpath('//div[contains(@class, "slide-info-title")]/text()') or
                root.xpath('//title/text()') or
                ["未知"]
            )
            vod_name = vod_name_candidates[0].strip()
            if ',' in vod_name or 'LreeOk' in vod_name:
                vod_name = vod_name.split(',')[0].strip() if ',' in vod_name else vod_name.split(' - ')[0].strip()
