...
class Spider(Spider):
    ...
    def categoryContent(self, cid, page, filter, ext):
        ...
        try:
            ...
            for item in data:
                ...
                lang_match = not ext.get('lang') or (vod_lang and ext['lang'] in vod_lang) or vod_lang == ''
    ...
    def detailContent(self, did):
        ...
        try:
            ...
            vod_year = api_data.get('vod_year', '')
            vod_area = ""
            vod_remarks = api_data.get('vod_remarks', '')
            vod_director = api_data.get('vod_director', '')
            vod_actor = api_data.get('vod_actor', '')
            vod_class = api_data.get('vod_class', '')
            vod_lang = ""
            vod_content = api_data.get('vod_content', '暫無簡介')

            for li in info_params:
                label = extract_fallback(li, './/em[@class="cor4"]/text()')
                if "年份：" in label:
                    vod_year = li.xpath('.//text()')[1].strip() if len(li.xpath('.//text()')) > 1 else vod_year
                elif "地區：" in label:
                    vod_area = li.xpath('.//text()')[1].strip() if len(li.xpath('.//text()')) > 1 else ""
                elif "狀態：" in label:
                    vod_remarks = extract_fallback(li, './/span/text()', vod_remarks)
                elif "導演：" in label:
                    vod_director = " / ".join(li.xpath('.//a/text()')) if li.xpath('.//a/text()') else vod_director
                elif "主演：" in label:
                    vod_actor = " / ".join(li.xpath('.//a/text()')) if li.xpath('.//a/text()') else vod_actor
                elif "類型：" in label:
                    vod_class = " ".join(li.xpath('.//a/text()')) if li.xpath('.//a/text()') else vod_class
                elif "語言：" in label:
                    vod_lang = li.xpath('.//text()')[1].strip() if len(li.xpath('.//text()')) > 1 else "国语"
                elif "簡介：" in label:
                    vod_content = li.xpath('.//text()')[1].strip() if len(li.xpath('.//text()')) > 1 else vod_content

            # 備用提取邏輯
            if not vod_year:
                vod_year = extract_fallback(root, '//div[@class="slide-info hide"]/span[@class="slide-info-remarks"]/a[contains(@href, "202")]/text()', "2023")
            if not vod_area:
                vod_area = extract_fallback(root, '//div[@class="slide-info hide"]/span[@class="slide-info-remarks"]/a[contains(@href, "%E5%A4%A7%E9%99%86")]/text()', "大陆")
            if not vod_lang:
                vod_lang = extract_fallback(root, '//div[@class="info-parameter"]//li[contains(.//em, "语言")]/text()[last()]', "国语")
            if not vod_director:
                vod_director = extract_fallback(root, '//span[contains(text(), "导演")]/following-sibling::a/text()', api_data.get('vod_director', ''))
            if not vod_actor:
                vod_actor = " / ".join(root.xpath('//span[contains(text(), "主演")]/following-sibling::a/text()')) if root.xpath('//span[contains(text(), "主演")]/following-sibling::a/text()') else api_data.get('vod_actor', '')
            if not vod_content or vod_content == '暫無簡介':
        ...
        try:
            ...
            for i in data_list:
                data.append({
                    ...
                    'vod_year': i.get('vod_year', ''),  # API 中可能無此字段，保持空值
                    'vod_actor': i.get('vod_actor', ''),
                    'vod_director': i.get('vod_director', ''),
                    'vod_content': i.get('vod_blurb', '')  # 使用 vod_blurb 作為簡介
                })
            return data
        except requests.RequestException as e:
            print(f"API 請求錯誤: {e}")
