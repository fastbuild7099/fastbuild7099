...
class Spider(Spider):
    ...
    def detailContent(self, did):
        ...
        try:
            ...
            # 強制使用 UTF-8 解碼，並處理可能的 BOM 或異常字元
            res.encoding = 'utf-8'
            ...
            # 清理異常字元，包括 BOM (ufeff)
            cleaned_text = re.sub(r'[\ufeff\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F]', '', res.text)
    ...
    def get_data(self, url_or_text):
        ...
        try:
            if isinstance(url_or_text, str) and url_or_text.startswith('http'):
                ...
                cleaned_text = re.sub(r'[\ufeff\x00-\x08\x0B\x0C\x0E-\x1F\x7F-\x9F]', '', url_or_text)
                soup = BeautifulSoup(cleaned_text, 'html.parser')
