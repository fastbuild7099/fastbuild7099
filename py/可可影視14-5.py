...
class Spider(Spider):
    ...
    def detailContent(self, did):
        ...
        try:
            ...
            if not play_lists:
                ...
                for i, play_list in enumerate(play_lists):
                    ...
                    if episode_names and episode_urls:
                        # 確保集數名稱和 URL 數量匹配
                        min_length = min(len(episode_names), len(episode_urls))
                        episode_list = [f"{episode_names[j]}${self.home_url}{episode_urls[j]}" 
            ...
            # 改進標題提取
            vod_name = soup.select_one('h1.detail-title')
            if vod_name:
                vod_name = vod_name.text.strip()
            else:
                vod_name = re.search(r'<h1[^>]*>([^<]+)</h1>', html)
                vod_name = vod_name.group(1).strip() if vod_name else '未找到標題'
            
            vod_content = soup.select_one('div.detail-desc')
            vod_content = vod_content.text.strip() if vod_content else ''
            
            # 改進年份和地區提取
            tags = soup.select('div.detail-tags-item span')
            vod_year = tags[0].text.strip() if tags else ''
            vod_area = tags[1].text.strip() if len(tags) > 1 else ''
            
            vod_actor = soup.find('div', string='演员:')
            vod_actor = vod_actor.find_next('div').text.strip() if vod_actor else ''
