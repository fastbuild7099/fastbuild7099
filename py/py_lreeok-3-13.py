...
class Spider(Spider):
    def __init__(self):
        ...
        self.detail_cache = {}
        self.cache_lock = threading.Lock()  # 添加鎖以確保緩存同步
    ...
    def categoryContent(self, cid, page, filter, ext):
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=5) as executor:
                futures = {executor.submit(self.detailContent, [item['vod_id']]): item for item in data}
                    ...
                    try:
                        ...
                        if result['list']:
                            with self.cache_lock:
                                self.detail_cache[item['vod_id']] = result['list'][0]
                            print(f"緩存更新: vod_id={item['vod_id']}, vod_area={result['list'][0]['vod_area']}")
                    except Exception as e:
                        print(f"獲取詳情頁失敗 for vod_id {item['vod_id']}: {e}")
            ...
            for item in data:
                ...
                # 優先使用緩存中的 vod_area，若無則使用 API 返回的默認值
                vod_area = cached_item.get('vod_area') if cached_item.get('vod_area') else item.get('vod_area', '大陆')
                print(f"檢查緩存: vod_id={vod_id}, cached_vod_area={cached_item.get('vod_area')}, api_vod_area={item.get('vod_area', '未知')}, final_vod_area={vod_area}")

                area_filter = ext.get('area', '')
                area_match = (
                    not area_filter or
                    area_filter == vod_area or
                    (area_filter == '美国' and '美国' in vod_area) or
                    (area_filter == '大陆' and '大陆' in vod_area) or
                    (area_filter == '英国' and '英国' in vod_area)
                )
    ...
    def detailContent(self, did):
        ...
        try:
            ...
            def extract_fallback(root, xpath, default=""):
                ...
                extracted = result[0].strip() if result else default
                print(f"XPath {xpath} 提取結果: {extracted}")
                return extracted
        ...
        try:
            ...
            for i in data_list:
                data.append({
                    ...
                    'vod_area': i.get('vod_area', '大陆')  # API默認值
                })
            return data
        except Exception as e:
            print(f"API 請求錯誤: {e}")
