...
class Spider(Spider):
    ...
    def detailContent(self, did):
        print(f"Debug: Received did parameter: {did}, type: {type(did)}")
        ...
        try:
            ...
            print(f"Debug: Raw content snippet: {cleaned_text[:200]}")
            ...
            vod_play_from = "未找到播放線路" if not source_items else '$$$'.join([span.text.strip() for span in source_items])
            ...
            print(f"Debug: Final result: {json.dumps(result, ensure_ascii=False)}")
            return result
        except Exception as e:
            print(f"Error in detailContent: {e}")
            print(f"Debug: Problematic content: {res.text[:200] if 'res' in locals() else 'No response'}")
            return {'list': [], 'msg': str  str(e)}
