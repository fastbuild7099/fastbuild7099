...
class Spider(Spider):
    ...
    def detailContent(self, did):
        ...
        try:
            ...
            play_from, play_url = [], []
            # 嘗試從播放選項卡提取真實線路名稱
            anthology_tabs = root.xpath('//div[contains(@class, "anthology-tab")]//a/@data-from')
            if not anthology_tabs:  # 如果未找到 data-from 屬性，嘗試其他結構
                anthology_tabs = root.xpath('//div[contains(@class, "anthology-tab")]//a/span/text()')
            if not anthology_tabs:  # 如果仍未找到，嘗試直接從文本提取
                anthology_tabs = root.xpath('//div[contains(@class, "anthology-tab")]//a/text()')
            ...
            for i, box in enumerate(anthology_boxes):
                # 使用提取到的真實線路名稱，如果失敗則使用描述性默認值
                source_name = anthology_tabs[i].strip().replace("\xa0", "") if i < len(anthology_tabs) else f"線路{i+1}"
